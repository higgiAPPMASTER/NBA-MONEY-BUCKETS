...
def _verify_hub_token(token: str) -> bool:
    if not JWT_SECRET:
        return True   # allow all if secret not configured
    try:
...
async def run_analysis(selected_date: str = None) -> Dict:
    ...
    if not games:
        return {'date': today_str, 'picks': [], 'games': [], 'log': [f'No NBA games found for {today_str}.'], 'total': 0}
...
async def index(request: Request):
    pass  # auth handled by hub JWT gate in frontend
    today_iso = date.today().isoformat()
    return HTMLResponse(MAIN_HTML.replace("__TODAY__", today_iso))

@app.get("/login")
async def login_get():
    return RedirectResponse("https://www.moneypicksarena.com", status_code=302)
