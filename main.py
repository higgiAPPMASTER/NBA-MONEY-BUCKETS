...
/* ── Card ── */
.card{
  background:linear-gradient(145deg,rgba(35,35,35,.97),rgba(30,30,30,.99));
  border:1px solid rgba(50,50,50,.8);border-radius:24px;
...
.sub{color:#555;font-size:.75rem;margin-bottom:30px;letter-spacing:1.5px;text-transform:uppercase}
...
input{
  width:100%;background:rgba(35,35,35,.8);
  border:1px solid rgba(50,50,50,.8);color:#d1d5db;
...
.tagline{color:#222;font-size:.68rem;margin-top:22px;letter-spacing:2px;text-transform:uppercase}
</style>
</head>
<body>
<script>
...
/* ─── Pick cards ─── */
.picks-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:14px;margin-bottom:16px}
.pick-card{
  background:#1e1e1e;
...
.pick-card:hover{border-color:#FDB827;transform:translateY(-2px);box-shadow:0 8px 28px rgba(0,0,0,.5),0 0 16px rgba(253,184,39,.15)}
/* Rank medals */
...
.rank-2{background:linear-gradient(135deg,#555,#9ca3af);color:#0d0d0d;box-shadow:0 0 8px rgba(156,163,175,.2)}
.rank-3{background:linear-gradient(135deg,#7c2d12,#c2410c);color:#fff0e0;box-shadow:0 0 10px rgba(194,65,12,.3)}
.rank-other{background:rgba(35,35,35,.8);color:#444;font-size:.75rem}
.pick-emoji{font-size:1.6rem;margin-bottom:10px;display:block}
.pick-player{font-size:1.08rem;font-weight:800;color:#f0e6c8;margin-bottom:3px;letter-spacing:-.3px;padding-right:38px}
.pick-team{font-size:.75rem;color:#555;margin-bottom:12px;display:flex;align-items:center;gap:6px}
.loc-badge{background:rgba(35,35,35,.8);padding:2px 9px;border-radius:10px;font-size:.7rem;color:#666;border:1px solid rgba(50,50,50,.5)}
...
.tag-pts{background:rgba(109,40,217,.25);color:#c4b5fd;border:1px solid rgba(109,40,217,.4)}
.tag-reb{background:rgba(37,99,235,.15);color:#FDB827;border:1px solid rgba(37,99,235,.25)}
.tag-ast{background:rgba(5,150,105,.25);color:#6ee7b7;border:1px solid rgba(5,150,105,.4)}
.tag-fg3m{background:rgba(220,38,38,.25);color:#fca5a5;border:1px solid rgba(220,38,38,.4)}
.pick-pattern{font-size:.9rem;color:#FDB827;font-weight:700;margin-bottom:4px;line-height:1.4}
.l10vthr-desc{font-size:.88rem;color:#FDB827;font-weight:700;margin-bottom:5px;line-height:1.4}
.fd-line-badge{display:inline-block;background:#1a1a0a;border:1px solid #22c55e;color:#22c55e;
...
.pick-matchup{font-size:.72rem;color:#444;margin-bottom:16px}
.bar-wrap{background:rgba(35,35,35,.7);border-radius:6px;height:8px;overflow:hidden;margin-bottom:10px;border:1px solid rgba(60,60,60,.3)}
...
.games-chip{background:rgba(35,35,35,.7);padding:4px 12px;border-radius:20px;font-size:.75rem;color:#444;border:1px solid rgba(60,60,60,.3)}
...
.game-group-hdr{
  ...
  background:#1a1a1a;
  border:1px solid rgba(50,50,50,.65);border-radius:13px;
...
.gg-chevron{color:#444;font-size:.85rem;transition:transform .2s}
.compact-picks{display:flex;flex-direction:column;gap:5px;margin-bottom:4px}
.compact-row{
  display:flex;align-items:center;gap:12px;
  background:#1e1e1e;border:1px solid #3a3a3a;border-radius:8px;padding:10px 15px;
  transition:border-color .2s,background .2s;
}
.compact-row:hover{border-color:rgba(253,184,39,.35);background:rgba(35,35,35,.9)}
.cr-emoji{font-size:1.05rem;flex-shrink:0;width:22px;text-align:center}
.cr-info{flex:1;min-width:0}
.cr-player{font-size:.86rem;font-weight:700;color:#ccc;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
...
.cr-bar-wrap{background:rgba(35,35,35,.8);border-radius:4px;height:4px;width:68px;overflow:hidden}
.cr-bar-fill{height:100%;border-radius:4px}
.cr-pct{font-size:.9rem;font-weight:900}
.cr-sample{font-size:.65rem;color:#444}
/* ─── Messages ─── */
.msg-card{
  background:#1a1a1a;
  border:1px solid #333;border-radius:16px;padding:60px 30px;text-align:center;
...
.msg-card p{color:#555;font-size:.88rem;line-height:1.75}
/* ─── Log ─── */
.log-box{background:rgba(20,20,20,.8);border:1px solid rgba(30,30,30,.8);border-radius:12px;padding:16px;font-size:.74rem;color:#444;font-family:'Courier New',monospace;margin-top:20px;max-height:160px;overflow-y:auto;line-height:1.9;scrollbar-width:thin;scrollbar-color:#222 transparent}
footer{text-align:center;margin-top:32px;color:#222;font-size:.68rem;padding:10px;letter-spacing:1.5px;text-transform:uppercase}
...
<div id="gamesBar" style="display:none"></div>
...
<div id="allPicksWrap" style="display:none">
  ...
  <div class="all-section-hdr">
    ...
    <div class="chip-filter" id="allFilterBar">
      <button class="chip-btn active" onclick="filterAll('ALL')"><span class="cv">ALL</span><span class="cl">Stats</span></button>
      <button class="chip-btn" onclick="filterAll('PTS')"><span class="cv">PTS</span><span class="cl">Points</span></button>
      <button class="chip-btn" onclick="filterAll('REB')"><span class="cv">REB</span><span class="cl">Rebounds</span></button>
      <button class="chip-btn" onclick="filterAll('AST')"><span class="cv">AST</span><span class="cl">Assists</span></button>
      <button class="chip-btn" onclick="filterAll('FG3M')"><span class="cv">3PM</span><span class="cl">3-Pointers</span></button>
    </div>
  </div>
...
function filterAll(stat){
  activeAllStat=stat;
  document.querySelectorAll('#allFilterBar .chip-btn').forEach(b=>{
    const t=b.textContent;
    b.classList.toggle('active',
...
function renderTop10Cards(picks){
  ...
  let html=`<div class="sec">TOP PICKS</div><div class="picks-grid">`;
  picks.forEach((p,i)=>{
  ...
  for(const matchup of order){
    ...
    for(const p of gp){
      ...
      html+=`<div class="compact-row">
        ...
        <div class="cr-info">
          <div class="cr-player">${p.player} <span style="color:#444;font-size:.65rem">${p.team}·${p.location==='Home'?'🏠':'✈️'}</span></div>
...
function renderGames(games){
  ...
  // gamesBar hidden
  if(false) document.getElementById('gamesBar').innerHTML=games.map(g=>
...
async function runPicks(){
  ...
  document.getElementById('content').innerHTML=`
    <div class="msg-card">
      ...
      <span style="color:#444">This takes ~45 seconds — worth the wait.</span></p>
    </div>`;
