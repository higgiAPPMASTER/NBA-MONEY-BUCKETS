...
header{padding:12px 20px!important;border-radius:8px!important;margin-bottom:14px!important;background:#111!important;border:none!important;border-bottom:3px solid #FDB827!important;box-shadow:none!important;position:static!important}
header::before{display:none!important}
.brand-text h1{font-size:1.4rem!important;font-weight:900!important;letter-spacing:1px!important;color:#FDB827!important;-webkit-text-fill-color:#FDB827!important;background:none!important}
.ball-svg{display:none!important}
.sub{display:none!important}
.brand-text .sub{display:none!important}
</style>
</head>
<body>
<script>(function(){var H='https://www.moneypicksarena.com',K='__mpa_token',p=new URLSearchParams(window.location.search),t=p.get('token');if(t){localStorage.setItem(K,t);window.history.replaceState({},'',window.location.pathname);}if(!localStorage.getItem(K)){window.location.href=H;}})();</script>
    ...
    <svg class="ball-svg" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100">
      <defs>
        <radialGradient id="ballG" cx="38%" cy="35%" r="65%">
          ...
          <stop offset="55%" stop-color="#FDB827"/>
...
function renderPropsSection(picks, nopick) {
  ...
  if(!ctr) return;
  if(!all.length){
    ctr.innerHTML='<p style="text-align:center;padding:20px;color:#555">No prop lines available yet.</p>';
    return;
  }
  var groups={}, order=[];
  ...
  order.forEach(function(matchup, mi){
    ...
    html += '<div style="background:#111;border-left:4px solid #FDB827;padding:9px 14px;cursor:pointer;display:flex;justify-content:space-between;align-items:center;border-radius:0 6px 6px 0" ';
    html += 'onclick="var e=document.getElementById(\''+gid+'\');e.style.display=e.style.display===\'none\'?\'block\':\'none\';">';
    html += '<span style="font-weight:900;color:#fff;font-size:.85rem">🏀 '+matchup+'</span>';
    html += '<span style="background:rgba(253,184,39,.15);border:1px solid rgba(253,184,39,.3);color:#FDB827;padding:2px 8px;border-radius:20px;font-size:.75rem">'+gp.length+' props ▾</span>';
    html += '</div>';
    html += '<div id="'+gid+'" style="overflow-x:auto;border:1px solid #222;border-top:none">';
    html += '<table style="width:100%;border-collapse:collapse;font-size:.82rem;background:#0d0d0d">';
    html += '<thead><tr style="border-bottom:2px solid #FDB827">';
    var headers=['Player','Stat','H/A','Line','Avg','Gap','Games','History','Pick'];
    headers.forEach(function(h){ html+='<th style="padding:8px 10px;text-align:left;color:#FDB827;font-size:.72rem;background:#111;white-space:nowrap">'+h+'</th>'; });
    html += '</tr></thead><tbody>';
    gp.forEach(function(p){
      ...
      html += '<td style="padding:8px 10px;font-weight:700">'+p.player+'</td>';
      html += '<td style="padding:8px 10px;color:#FDB827;font-size:.8rem">'+p.emoji+' '+p.stat_label+'</td>';
      html += '<td style="padding:8px 10px"><span style="background:'+sideBg+';padding:2px 6px;border-radius:4px;font-size:.75rem">'+p.side+'</span></td>';
      html += '<td style="padding:8px 10px;font-family:monospace;font-weight:700">'+p.line+'</td>';
      html += '<td style="padding:8px 10px;font-family:monospace;color:'+clr+';font-weight:700">'+(p.avg!=null?p.avg:'—')+'</td>';
      html += '<td style="padding:8px 10px;font-family:monospace;color:'+clr+';font-weight:700">'+gap+'</td>';
      html += '<td style="padding:8px 10px;color:#555">'+p.games+'g</td>';
      html += '<td style="padding:8px 10px;font-size:.7rem;color:#555">'+p.history+'</td>';
      html += '<td style="padding:8px 10px"><span style="color:'+clr+';font-weight:900">'+(p.pick||'—')+'</span></td>';
      html += '</tr>';
    });
