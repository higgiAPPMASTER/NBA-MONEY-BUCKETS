    ...
    for game in games:
        ...
        for loc,tid,opp_id,opp_name,side in [('Home',game['home_id'],a,a_name,'HOME'),('Away',game['away_id'],h,h_name,'AWAY')]:
            for player in rosters.get(tid,[]):
                ...
                for sk,sc in STAT_CONFIG.items():
                    # Prefer Underdog (real sportsbook O/U) over OddsAPI (may have alternate/quarter lines)
...
function renderPropsSection(picks, nopick) {
  ...
  if (all.length === 0) {
    sec.querySelector('#props-body') && (sec.querySelector('#props-body').innerHTML = '');
    document.getElementById('props-game-groups').innerHTML =
      '<div style="text-align:center;padding:20px;color:#555">No prop lines available yet — check back closer to tip-off.</div>';
    return;
  }
  // Group by matchup
  var groups = {}, order = [];
  all.forEach(function(p) {
    var matchup = p.side==='HOME'
      ? (p.opp_name + ' @ ' + (p.team||p.opp_name))
      : ((p.team||p.player) + ' @ ' + p.opp_name);
    if (!groups[matchup]) { groups[matchup]=[]; order.push(matchup); }
    groups[matchup].push(p);
  });
  var html = '';
  order.forEach(function(matchup, mi) {
    var gp = groups[matchup];
    var gid = 'pg_'+mi;
    html += '<div class="game-group">';
    html += '<div class="game-group-hdr" onclick="toggleGroup(''+gid+'',this)">';
    html += '<span class="gg-label">🏀 '+matchup+'</span>';
    html += '<div class="gg-meta"><span class="count-pill">'+gp.length+' prop'+(gp.length!==1?'s':'')+'</span><span class="gg-chevron">▾</span></div>';
    html += '</div>';
    html += '<div id="'+gid+'" style="overflow-x:auto;border-radius:8px;border:1px solid #222;margin-bottom:8px">';
    ...
    html += '<th style="padding:9px 12px;text-align:left;color:#FDB827;font-size:.72rem;background:#111">Player</th>';
    html += '<th style="padding:9px 12px;text-align:left;color:#FDB827;font-size:.72rem;background:#111">Stat</th>';
    html += '<th style="padding:9px 12px;text-align:left;color:#FDB827;font-size:.72rem;background:#111">H/A</th>';
    html += '<th style="padding:9px 12px;text-align:left;color:#FDB827;font-size:.72rem;background:#111">Line</th>';
    html += '<th style="padding:9px 12px;text-align:left;color:#FDB827;font-size:.72rem;background:#111">Avg vs Opp</th>';
    html += '<th style="padding:9px 12px;text-align:left;color:#FDB827;font-size:.72rem;background:#111">Gap</th>';
    html += '<th style="padding:9px 12px;text-align:left;color:#FDB827;font-size:.72rem;background:#111">Games</th>';
    html += '<th style="padding:9px 12px;text-align:left;color:#FDB827;font-size:.72rem;background:#111">History</th>';
    html += '<th style="padding:9px 12px;text-align:left;color:#FDB827;font-size:.72rem;background:#111">Pick</th>';
    html += '</tr></thead><tbody>';
    gp.forEach(function(p) {
      ...
      html += '<td style="padding:9px 12px;font-weight:700">'+p.player+'</td>';
      html += '<td style="padding:9px 12px;color:#FDB827;font-size:.8rem">'+p.emoji+' '+p.stat_label+'</td>';
      html += '<td style="padding:9px 12px"><span style="background:'+sideBg+';padding:2px 7px;border-radius:4px;font-size:.75rem">'+p.side+'</span></td>';
      html += '<td style="padding:9px 12px;font-family:monospace;font-weight:700">'+p.line+'</td>';
      html += '<td style="padding:9px 12px;font-family:monospace;font-weight:700;color:'+clr+';">'+(p.avg!=null?p.avg:'—')+'</td>';
      html += '<td style="padding:9px 12px;font-family:monospace;color:'+clr+';font-weight:700">'+gap+'</td>';
      html += '<td style="padding:9px 12px;color:#555">'+p.games+'g</td>';
      html += '<td style="padding:9px 12px;font-family:monospace;font-size:.7rem;color:#555;max-width:130px">'+p.history+'</td>';
      html += '<td style="padding:9px 12px"><span style="color:'+clr+';font-weight:900;font-size:.95rem">'+(p.pick||'—')+'</span></td>';
      html += '</tr>';
    });
    html += '</tbody></table></div></div>';
  });
  document.getElementById('props-game-groups').innerHTML = html;
}
</script>
</body>
</html>"""
