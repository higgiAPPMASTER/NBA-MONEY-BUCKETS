...
body{
  background:#0f0f0f;
  background-image:radial-gradient(ellipse at 50% 40%,rgba(245,158,11,.04),transparent 65%);
...
.spin-ball{
  ...
  background:radial-gradient(circle at 38% 35%,#f59e0b 0%,#f59e0b 55%,#7c2d12 100%);
...
input:focus{border-color:#f59e0b;box-shadow:0 0 0 3px rgba(253,184,39,.12)}
input::placeholder{color:#374151}
.btn-in{
  width:100%;margin-top:8px;
  background:linear-gradient(135deg,#f59e0b,#f59e0b);color:#0d0d0d;
...
/* ─── Reset ─── */
*{box-sizing:border-box;margin:0;padding:0}
:root{--orange:#f59e0b;--gold:#f59e0b;--blue:#f59e0b;--green:#22c55e;--navy:#0a0f1e;--dark:#0d0d0d;--card:#0f172a;--border:#1e3a5f;--text:#e0e6f0;--muted:#4b5563}
body{
  background:var(--dark);
  background-image:
  ...
  color:var(--text);font-family:'Source Sans Pro',sans-serif;padding:20px;min-height:100vh;
}
...
@keyframes spinBall{from{transform:rotate(0)}to{transform:rotate(360deg)}}
/* Loading bounce */
.loading-ball{width:58px;height:58px;border-radius:50%;margin:0 auto 6px;animation:ballBounce .65s ease-in-out infinite;background:radial-gradient(circle at 38% 35%,#f59e0b 0%,#f59e0b 55%,#7c2d12 100%);border:2px solid #7c2d12;box-shadow:0 0 25px rgba(253,184,39,.45);position:relative}
...
header::before{content:'';position:absolute;top:0;left:0;right:0;height:2px;background:linear-gradient(90deg,transparent,#f59e0b 30%,#f59e0b 70%,transparent)}
...
.date-badge{background:rgba(42,42,42,.5);color:#f59e0b;padding:7px 16px;border-radius:20px;font-size:.81rem;font-weight:600;border:1px solid rgba(253,184,39,.2)}
...
.btn-run{background:linear-gradient(135deg,#f59e0b,#f59e0b);color:#0d0d0d;box-shadow:0 4px 18px rgba(253,184,39,.35)}
...
.fd-indicator{display:flex;align-items:center;gap:6px;background:rgba(15,23,42,.7);
  border:1px solid rgba(253,184,39,.2);border-radius:20px;padding:5px 12px;cursor:default}
...
.filter-btn.active,.filter-btn:hover{background:rgba(42,42,42,.9);color:#fff;border-color:rgba(253,184,39,.4);box-shadow:0 0 12px rgba(253,184,39,.1)}
/* ─── Section headers ─── */
.section-hdr{display:flex;align-items:center;justify-content:space-between;margin-bottom:18px}
.section-title{font-size:1rem;font-weight:900;letter-spacing:-.3px;display:flex;align-items:center;gap:8px;background:linear-gradient(135deg,#f59e0b,#f59e0b);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text}
.count-pill{background:rgba(42,42,42,.5);color:#f59e0b;padding:4px 14px;border-radius:20px;font-size:.78rem;font-weight:700;border:1px solid rgba(253,184,39,.2)}
/* ─── Pick cards ─── */
...
.rank-1{background:linear-gradient(135deg,#92400e,#f59e0b);color:#0d0d0d;box-shadow:0 0 14px rgba(253,184,39,.5)}
...
.tag-reb{background:rgba(37,99,235,.15);color:#f59e0b;border:1px solid rgba(37,99,235,.25)}
...
.bar-yellow{background:linear-gradient(90deg,#f59e0b,#f59e0b)}
.bar-orange{background:linear-gradient(90deg,#c2410c,#f59e0b)}
...
.pct-yellow{color:#f59e0b;text-shadow:0 0 14px rgba(253,184,39,.45)}
.pct-orange{color:#f59e0b;text-shadow:0 0 12px rgba(253,184,39,.35)}
/* ─── Total Banner ─── */
.total-banner{
...
.all-section-title{font-size:.95rem;font-weight:800;color:#f59e0b;display:flex;align-items:center;gap:8px}
...
.gg-label{font-size:.88rem;font-weight:800;color:#fff;display:flex;align-items:center;gap:8px}
...
.cr-pattern{font-size:.76rem;color:#f59e0b;font-weight:600;margin-top:2px}
...
.msg-card h2{color:#fff;font-size:1.2rem;font-weight:800;margin-bottom:10px}
...
<nav style="position:fixed;top:0;width:100%;background:rgba(10,10,10,.95);backdrop-filter:blur(12px);border-bottom:1px solid #1c1c1c;z-index:100;padding:0 28px;height:72px;display:flex;align-items:center;justify-content:space-between"><span style="font-family:'Playfair Display',serif;font-size:1.5rem;font-weight:900;color:#f59e0b">Money<span style="color:#fff">Buckets</span> 🏀</span><span style="font-size:12px;color:#6b7280">NBA Pattern Picks</span></nav>

<div class="run-box" id="runBox" style="max-width:600px;margin:0 auto 20px"><h2>Run Picks</h2><p>Pick a date and run the algorithm.</p><div class="date-row"><label>DATE</label><input type="date" id="datePicker" max=""/></div><button class="btn-run" id="runBtn" onclick="runPicks()">&#9889; RUN PICKS</button></div><div class="games-bar" id="gamesBar">
</div>
...
<div id="props-section" style="display:none;margin-top:28px;max-width:1300px;margin:28px auto 0;padding:0 20px 20px">
  <div style="font-size:.85rem;font-weight:900;letter-spacing:2px;text-transform:uppercase;background:#111;border-left:4px solid #f59e0b;padding:10px 16px;color:#fff;margin-bottom:14px;border-radius:0 6px 6px 0">⚡ Player Props vs Opponent History</div>
  <div style="overflow-x:auto;border-radius:8px;border:1px solid #222">
    <table style="width:100%;border-collapse:collapse;font-size:.82rem;background:#0f0f0f">
      <thead><tr style="border-bottom:2px solid #f59e0b">
        <th style="padding:10px 12px;text-align:left;color:#f59e0b;font-size:.72rem;background:#111;white-space:nowrap">#</th>
        <th style="padding:10px 12px;text-align:left;color:#f59e0b;font-size:.72rem;background:#111;white-space:nowrap">Player</th>
        <th style="padding:10px 12px;text-align:left;color:#f59e0b;font-size:.72rem;background:#111;white-space:nowrap">Stat</th>
        <th style="padding:10px 12px;text-align:left;color:#f59e0b;font-size:.72rem;background:#111;white-space:nowrap">H/A</th>
        <th style="padding:10px 12px;text-align:left;color:#f59e0b;font-size:.72rem;background:#111;white-space:nowrap">Opponent</th>
        <th style="padding:10px 12px;text-align:left;color:#f59e0b;font-size:.72rem;background:#111;white-space:nowrap">Line</th>
        <th style="padding:10px 12px;text-align:left;color:#f59e0b;font-size:.72rem;background:#111;white-space:nowrap">Avg vs Opp</th>
        <th style="padding:10px 12px;text-align:left;color:#f59e0b;font-size:.72rem;background:#111;white-space:nowrap">Gap</th>
        <th style="padding:10px 12px;text-align:left;color:#f59e0b;font-size:.72rem;background:#111;white-space:nowrap">Games</th>
        <th style="padding:10px 12px;text-align:left;color:#f59e0b;font-size:.72rem;background:#111;white-space:nowrap">History</th>
        <th style="padding:10px 12px;text-align:left;color:#f59e0b;font-size:.72rem;background:#111;white-space:nowrap">Pick</th>
      </tr></thead>
  ...
  <p style="font-size:.72rem;color:#555;margin-top:8px">
    ...
    <strong style="color:#f59e0b">Pick</strong> = OVER if avg &gt; line, UNDER if avg &lt; line
  </p>
</div>
...
async function runPicks(){
  ...
  document.getElementById('content').innerHTML=`
    <div class="msg-card">
      ...
      <p>Pulling data for <strong style="color:#f59e0b">${selectedDate}</strong> from NBA Stats API.<br>
...
function renderPropsSection(picks, nopick) {
  ...
  body.innerHTML = all.map(function(p,i) {
    ...
    return '<tr style="border-bottom:1px solid #1a1a1a">' +
      ...
      '<td style="padding:9px 12px;color:#f59e0b;font-size:.8rem">'+p.emoji+' '+p.stat_label+'</td>' +
