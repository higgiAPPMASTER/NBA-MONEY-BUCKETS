...
/* ─── Reset ─── */
*{box-sizing:border-box;margin:0;padding:0}
:root{--orange:#FDB827;--gold:#FDB827;--blue:#FDB827;--green:#22c55e;--navy:#0a0f1e;--dark:#0d0d0d;--card:#0f172a;--border:#1e3a5f;--text:#e0e6f0;--muted:#4b5563}
body{
  background:var(--dark);
  background-image:
...
@keyframes spinBall{from{transform:rotate(0)}to{transform:rotate(360deg)}}
/* Loading bounce */
.loading-ball{width:58px;height:58px;border-radius:50%;margin:0 auto 6px;animation:ballBounce .65s ease-in-out infinite;background:radial-gradient(circle at 38% 35%,#FDB827 0%,#FDB827 55%,#7c2d12 100%);border:2px solid #7c2d12;box-shadow:0 0 25px rgba(253,184,39,.45);position:relative}
...
.fd-dot.checking{background:#FDB827;animation:pulse-gold .8s infinite}
...
<nav style="position:fixed;top:0;width:100%;background:#000;border-bottom:3px solid #FDB827;z-index:100;padding:0 24px;height:60px;display:flex;align-items:center;justify-content:space-between"><span style="font-size:1.4rem;font-weight:900;color:#FDB827;letter-spacing:2px;text-transform:uppercase">Money <span style="color:#fff">Buckets</span> 🏀</span><span style="font-size:11px;color:#555;letter-spacing:1px;text-transform:uppercase">NBA Pattern Picks</span></nav><div style="height:72px"></div><div style="max-width:1300px;margin:0 auto;padding:0 20px 20px"><div style="background:#111;border:2px solid #FDB827;border-radius:10px;padding:24px;text-align:center;margin-bottom:24px"><div style="display:flex;align-items:center;justify-content:center;gap:14px;flex-wrap:wrap"><input type="date" id="datePicker" value="__TODAY__" style="background:#1a1a1a;color:#FDB827;border:1px solid #333;border-radius:6px;padding:9px 14px;font-size:.95rem;cursor:pointer;outline:none"><button class="btn btn-run" onclick="runPicks()">⚡ RUN PICKS</button></div></div>
...
function renderPropsSection(picks, nopick) {
  ...
  if(!ctr)return;
  if(!all.length){ctr.innerHTML='<p style="text-align:center;padding:20px;color:#555">No prop lines yet.</p>';return;}
  var groups={},order=[];
  ...
  order.forEach(function(matchup,mi){
    ...
    html+='<div style="margin-bottom:12px"><div style="background:#111;border-left:4px solid #FDB827;padding:10px 16px;cursor:pointer;display:flex;justify-content:space-between;align-items:center;border-radius:0 6px 6px 0" onclick="var e=document.getElementById(''+gid+'');e.style.display=e.style.display==='none'?'block':'none';">';
    html+='<span style="font-weight:900;color:#fff;font-size:.85rem">🏀 '+matchup+'</span><span style="background:rgba(253,184,39,.15);border:1px solid rgba(253,184,39,.3);color:#FDB827;padding:2px 10px;border-radius:20px;font-size:.75rem">'+gp.length+' props ▾</span></div>';
    html+='<div id="'+gid+'" style="overflow-x:auto;border:1px solid #222;border-top:none"><table style="width:100%;border-collapse:collapse;font-size:.82rem;background:#0d0d0d"><thead><tr style="border-bottom:2px solid #FDB827">';
    ['Player','Stat','H/A','Line','Avg','Gap','Games','History','Pick'].forEach(function(h){html+='<th style="padding:8px 12px;text-align:left;color:#FDB827;font-size:.72rem;background:#111;white-space:nowrap">'+h+'</th>';});
    html+='</tr></thead><tbody>';
    gp.forEach(function(p){
      var clr=p.pick==='OVER'?'#4ade80':p.pick==='UNDER'?'#f87171':'#555';
      var gap=p.gap!=null?(p.gap>0?'+':'')+p.gap:'—';
      html+='<tr style="border-bottom:1px solid #1a1a1a">';
      html+='<td style="padding:8px 12px;font-weight:700">'+p.player+'</td><td style="padding:8px 12px;color:#FDB827;font-size:.8rem">'+p.emoji+' '+p.stat_label+'</td>';
      html+='<td style="padding:8px 12px;font-size:.75rem">'+p.side+'</td><td style="padding:8px 12px;font-family:monospace;font-weight:700">'+p.line+'</td>';
      html+='<td style="padding:8px 12px;font-family:monospace;color:'+clr+';font-weight:700">'+(p.avg!=null?p.avg:'—')+'</td>';
      html+='<td style="padding:8px 12px;font-family:monospace;color:'+clr+';font-weight:700">'+gap+'</td>';
      html+='<td style="padding:8px 12px;color:#555">'+p.games+'g</td><td style="padding:8px 12px;font-size:.7rem;color:#555">'+p.history+'</td>';
      html+='<td style="padding:8px 12px"><span style="color:'+clr+';font-weight:900">'+(p.pick||'—')+'</span></td></tr>';
    });
