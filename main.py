...
# ─── PrizePicks API (free, no key needed) ────────────────────────────────────
PRIZEPICKS_STAT_MAP = {
    "Points":      "PTS",
    "Rebounds":    "REB",
    "Assists":     "AST",
    "3-PT Made":   "FG3M",
    "Pts+Reb+Ast": None,   # skip combos
    "Steals":      None,
    "Blocks":      None,
}

async def get_prizepicks_lines():
    """Fetch NBA player prop lines from PrizePicks — free, no API key needed."""
    props = []
    try:
        async with httpx.AsyncClient(timeout=20, headers={
            ...
            if r.status_code != 200:
                print(f"[PrizePicks] status {r.status_code}")
                return []
            data    = r.json()
            ...
            for proj in data.get("data", []):
                ...
                stat     = PRIZEPICKS_STAT_MAP.get(stat_raw)
                if not stat:
                    continue
                ...
                p_id  = rels.get("new_player", {}).get("data", {}).get("id", "")
                name  = player_map.get(p_id, attrs.get("description", ""))
                if not name:
                    continue
                props.append({
                    "player": name,
            ...
            print(f"[PrizePicks] {len(props)} NBA prop lines fetched")
    except Exception as e:
...
async def run_analysis(selected_date: str = None) -> Dict:
    ...
    try:
        games, pp_props, odds_props_raw = await asyncio.gather(
            get_today_games(today_str),
            get_prizepicks_lines(),
            get_odds_lines(today_str),
        )
        # Merge: PrizePicks first, fill gaps with Odds API
        seen_keys = {f"{p['player']}|{p['stat']}" for p in pp_props}
        odds_extras = [p for p in odds_props_raw if f"{p['player']}|{p['stat']}" not in seen_keys]
        odds_props = pp_props + odds_extras
        log.append(f"PrizePicks: {len(pp_props)} lines | Odds API: {len(odds_extras)} extra lines")
    except Exception as e:
...
function renderPropsSection(picks, nopick) {
  ...
  if (all.length === 0) {
    body.innerHTML = '<tr><td colspan="11" style="text-align:center;padding:20px;color:#6b7280">No prop lines available today — lines may not be posted yet or no H/A history vs today\'s opponent found.</td></tr>';
    return;
  }
