...
<body>
<script>(function(){var HUB='https://www.moneypicksarena.com',KEY='__mpa_token',p=new URLSearchParams(window.location.search),t=p.get('token');if(t){localStorage.setItem(KEY,t);window.history.replaceState({},'',window.location.pathname);}if(!localStorage.getItem(KEY)){window.location.href=HUB;}})();</script>
...
function renderPropsSection(picks, nopick) {
  ...
  order.forEach(function(matchup,mi){
    ...
    html+='<div style="margin-bottom:10px"><div style="background:#111;border-left:4px solid #FDB827;padding:9px 14px;cursor:pointer;display:flex;justify-content:space-between;align-items:center;border-radius:0 6px 6px 0" onclick="var e=document.getElementById(\''+gid+'\');e.style.display=e.style.display===\'none\'?\'block\':\'none\';">'+
    '<span style="font-weight:900;color:#fff;font-size:.85rem">🏀 '+matchup+'</span><span style="background:rgba(253,184,39,.15);border:1px solid rgba(253,184,39,.3);color:#FDB827;padding:2px 8px;border-radius:20px;font-size:.75rem">'+gp.length+' props ▾</span></div>';
    ...
    ['Player','Stat','H/A','Line','Avg','Gap','Games','History','Pick'].forEach(function(h){html+='<th style="padding:8px 10px;text-align:left;color:#FDB827;font-size:.72rem;background:#111;white-space:nowrap">'+h+'</th>';});
    html+='</tr></thead><tbody>';
    gp.forEach(function(p){
      var clr=p.pick==='OVER'?'#4ade80':p.pick==='UNDER'?'#f87171':'#555';
      var gap=p.gap!=null?(p.gap>0?'+':'')+p.gap:'—';
      html+='<tr style="border-bottom:1px solid #1a1a1a"><td style="padding:8px 10px;font-weight:700">'+p.player+'</td><td style="padding:8px 10px;color:#FDB827;font-size:.8rem">'+p.emoji+' '+p.stat_label+'</td><td style="padding:8px 10px;font-size:.75rem">'+p.side+'</td><td style="padding:8px 10px;font-family:monospace;font-weight:700">'+p.line+'</td><td style="padding:8px 10px;font-family:monospace;color:'+clr+';font-weight:700">'+( p.avg!=null?p.avg:'—')+'</td><td style="padding:8px 10px;font-family:monospace;color:'+clr+';font-weight:700">'+gap+'</td><td style="padding:8px 10px;color:#555">'+p.games+'g</td><td style="padding:8px 10px;font-size:.7rem;color:#555">'+p.history+'</td><td style="padding:8px 10px"><span style="color:'+clr+';font-weight:900">'+( p.pick||'—')+'</span></td></tr>';
    });
