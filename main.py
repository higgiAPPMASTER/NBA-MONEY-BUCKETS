...
async def fetch_fd_nba_lines() -> Dict[str, Dict]:
    """Fetch NBA player prop lines from FanDuel.
    Gets today's game event IDs then pulls player props per game.
    """
    ...
    try:
        ...
        async with CFSession(impersonate="chrome120") as s:
            ...
            r = await s.get(
                ...
                params={"page":"SPORT","sport":"NBA","_ak":"FhMFpcPWXMeyZxOx"},
                headers=hdrs, timeout=20)

            if r.status_code != 200:
                print(f"[FanDuel NBA] Sport page status {r.status_code}")
                return {}

            sport_data = r.json()
            event_ids  = list(sport_data.get("attachments",{}).get("events",{}).keys())
            print(f"[FanDuel NBA] {len(event_ids)} events found")

            if not event_ids:
                # Try parsing markets directly from sport page as fallback
                lines = _parse_fd_markets(sport_data)
                if lines:
                    return lines
                print("[FanDuel NBA] No events found on sport page")
                return {}

            # Step 2: Fetch player props for each event
            for eid in event_ids[:10]:  # limit to 10 games
                try:
                    r2 = await s.get(
                        "https://sbapi.tn.sportsbook.fanduel.com/api/event-page",
                        params={"eventId": eid, "_ak": "FhMFpcPWXMeyZxOx"},
                        headers=hdrs, timeout=15)
                    if r2.status_code != 200:
                        continue
                    event_lines = _parse_fd_markets(r2.json())
                    lines.update(event_lines)
                except Exception as e:
                    print(f"[FanDuel NBA] Event {eid} error: {e}")
                    continue
        ...
        print(f"[FanDuel NBA] fetch error: {e}")
        return {}
...
async def run_analysis(selected_date: str = None) -> Dict:
    ...
    if _cache.get('date') == today_str and _cache.get('picks') is not None and _cache.get('fd_loaded'):
        return _cache

    log = []
    log.append(f"📅 Fetching NBA schedule for {today_str} via ESPN...")

    # Fetch FanDuel lines concurrently with games
    fd_lines_task = asyncio.create_task(fetch_fd_nba_lines())

    try:
        games = await get_today_games(today_str)
    except Exception as e:
        return {'date': today_str, 'picks': [], 'all_picks': [], 'games': [],
                'log': [f'ESPN schedule error: {e}'], 'total': 0}
    ...
    # Fetch game logs per player from ESPN (3 seasons)
    all_player_ids = list({p['id'] for players in rosters.values() for p in players})
    log.append(f"📊 Fetching game logs for {len(all_player_ids)} players × {len(ESPN_SEASONS)} seasons...")
    ...
    for game in games:
        ...
        # Home players: look for historical HOME games vs today's away opponent
        ...
        # Away players: look for historical AWAY games vs today's home opponent
    ...
    # Attach FanDuel lines
    try:
        fd_lines = await fd_lines_task
        if fd_lines:
            log.append(f"🏙️ FanDuel lines loaded for {len(fd_lines)} players")
            top_picks  = attach_fd_lines(top_picks, fd_lines)
            picks      = attach_fd_lines(picks, fd_lines)
        else:
            log.append("⚠️ FanDuel lines unavailable — check FD_EMAIL/FD_PASSWORD in Render")
            for p in top_picks + picks:
                p["fd_line"] = None
    except Exception as e:
        log.append(f"⚠️ FanDuel error: {e}")
        for p in top_picks + picks:
            p["fd_line"] = None

    # ── L10VTHR: Last 10 vs Team Hit Rate against FanDuel line ──
    for pick in picks:   # top_picks share same objects, both updated
        fd_line = pick.get("fd_line")
        pid     = pick.get("player_id")
        if fd_line is not None and pid:
            opp_logs = [l for l in logs_by_player.get(pid, [])
                        if l["location"] == pick["location"] and l["opp"] == pick["opp"]][:10]
            if opp_logs:
                hits = sum(1 for l in opp_logs if float(l.get(pick["stat"], 0)) > fd_line)
                pick["l10vthr_hits"]  = hits
                pick["l10vthr_games"] = len(opp_logs)
            else:
                pick["l10vthr_hits"]  = None
                pick["l10vthr_games"] = None
        else:
            pick["l10vthr_hits"]  = None
            pick["l10vthr_games"] = None

    fd_loaded = bool(fd_lines) if 'fd_lines' in dir() else False
    result = {'date': today_str, 'picks': top_picks, 'all_picks': picks,
              'games': games, 'log': log, 'total': len(picks), 'fd_loaded': fd_loaded}
...
function renderTop10Cards(picks){
  ...
  picks.forEach((p,i)=>{
    ...
    <div class="pick-card">
      ...
      ${(p.fd_line !== null && p.fd_line !== undefined && p.l10vthr_hits !== null && p.l10vthr_hits !== undefined) ? `<div class="l10vthr-desc">${Math.ceil(p.fd_line)}+ ${p.stat_label} in ${p.l10vthr_hits} of ${p.l10vthr_games} ${p.location.toLowerCase()} games vs ${p.opp}</div>` : ''}
      ${p.fd_line !== null && p.fd_line !== undefined ? `<div class="fd-line-badge">🏙️ FanDuel Line: <strong>${p.fd_line}</strong></div>` : ''}
