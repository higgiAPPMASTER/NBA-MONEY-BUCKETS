...
async def run_analysis(selected_date: str = None) -> Dict:
    ...
    try:
        pp, odds_raw = await asyncio.gather(get_prizepicks_lines(), get_odds_lines(today_str))
        ...
        log.append(f"PrizePicks: {len(pp)} | OddsAPI: {len(odds_props)-len(pp)} lines")
    except Exception as e:
    ...
    for prop in odds_raw:
        key = (_nn(prop['player']), prop['stat'])
        dk_lookup[key] = {'line': prop['line'], 'odds': str(prop.get('odds', ''))}
            ...
            for sk, sc in STAT_CONFIG.items():
                ...
                if result:
                    ...
                    picks.append({**result, 'player': pname, 'player_id': pid, 'team': h,
                                  ...
                                  'dk_line': dk_line, 'dk_odds': dk_odds, 'dk_hits': dk_hits})
...
function renderTop10Cards(picks){
  ...
  picks.forEach((p,i)=>{
    ...
    <div class="pick-card">
      ...
      ${p.dk_line ? `<div class="fd-line-badge" style="background:rgba(99,102,241,.15);border-color:rgba(99,102,241,.3);margin-top:4px">🏙️ DraftKings: <strong>${p.dk_line} O/U</strong> ${p.dk_odds ? "(" + p.dk_odds + ")" : ""} | Hit ${p.dk_line}+: ${p.dk_hits !== null && p.dk_hits !== undefined ? p.dk_hits + "/" + p.l10_games + " vs " + p.opp : "—"}</div>` : ""}
