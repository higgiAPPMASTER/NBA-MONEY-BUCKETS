...
async def run_analysis(selected_date: str = None) -> Dict:
    ...
    for game in games:
        h, a = game['home'], game['away']
        h_name, a_name = game['home_name'], game['away_name']
        for loc, tid, opp_id, opp_name, side in [
            ('Home', game['home_id'], a, a_name, 'HOME'),
            ('Away', game['away_id'], h, h_name, 'AWAY'),
        ]:
            for player in rosters.get(tid, []):
                ...
                for sk, sc in STAT_CONFIG.items():
                    ...
                    if not opp_logs:
                        props_nopick.append({'player':pname,'stat':sk,'stat_label':sc['label'],
                            'emoji':sc['emoji'],'side':side,'opp_name':opp_name,'line':line,
                            'avg':None,'games':0,'history':'—','gap':None,'pick':None,
                            'pick_note':f'No {loc} history vs {opp_name}','fd_odds':ob.get('odds','')})
                        continue
                    vals2 = [float(l[sk]) for l in opp_logs]
                    avg = round(sum(vals2)/len(vals2), 1)
                    history = ', '.join(str(int(v)) for v in vals2[:8])
                    gap = round(avg - line, 1)
                    pick = 'OVER' if avg > line else ('UNDER' if avg < line else None)
                    entry = {'player':pname,'stat':sk,'stat_label':sc['label'],'emoji':sc['emoji'],
                        'side':side,'opp_name':opp_name,'line':line,'avg':avg,'games':len(vals2),
                        'history':history,'gap':gap,'pick':pick,
                        'pick_note':f'avg {avg} vs line {line}','fd_odds':ob.get('odds',''),
                        'matchup':f'{a_name} @ {h_name}'}
...
<div id="props-section" style="display:none;margin-top:28px;max-width:1300px;margin-left:auto;margin-right:auto;padding:0 20px">
  ...
  <p style="font-size:.72rem;color:#6b7280;margin-top:10px;padding-bottom:20px">
