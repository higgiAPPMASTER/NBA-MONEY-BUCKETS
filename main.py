...
<div id="props-section" style="display:none;margin-top:28px;max-width:1300px;margin:28px auto 0;padding:0 20px 20px">
  ...
  <div style="overflow-x:auto;border-radius:8px;border:1px solid #222;display:none">
    <table style="width:100%;border-collapse:collapse;font-size:.82rem;background:#0d0d0d">
      ...
      <tbody id="props-body" style="display:none"></tbody>
    </table>
  </div>
...
function renderPropsSection(picks, nopick) {
  ...
  var container = document.getElementById('props-game-groups');
  if (!container) return;
  if (!all.length) {
    container.innerHTML = '<p style="text-align:center;padding:20px;color:#555">No prop lines available yet.</p>';
    return;
  }
  var groups={}, order=[];
  ...
  order.forEach(function(matchup,mi){
    var gp=groups[matchup], gid='pg_'+mi;
    html+='<div class="game-group">';
    ...
    html+='</div><div id="'+gid+'" style="overflow-x:auto;border:1px solid #222;border-radius:8px;margin-bottom:8px">';
    ...
    ['Player','Stat','H/A','Line','Avg vs Opp','Gap','Games','History','Pick'].forEach(function(h){
      html+='<th style="padding:9px 12px;text-align:left;color:#FDB827;font-size:.72rem;background:#111;white-space:nowrap">'+h+'</th>';
    });
    html+='</tr></thead><tbody>';
    gp.forEach(function(p){
      ...
      html+='<td style="padding:9px 12px;font-size:.7rem;color:#555">'+p.history+'</td>';
      html+='<td style="padding:9px 12px"><span style="color:'+clr+';font-weight:900">'+(p.pick||'—')+'</span></td>';
      html+='</tr>';
    });
