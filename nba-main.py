...
# ─── NBA FanDuel Props ────────────────────────────────────────────────────────
# Maps our stat keys to FanDuel market name fragments
FD_MARKET_MAP = {
    "PTS":  ["points"],
    "REB":  ["rebounds", "rebound"],
    "AST":  ["assists", "assist"],
    "FG3M": ["threes", "three", "3-point", "3pt"],
}
        ...
        async with CFSession(impersonate="chrome120") as s:
            ...
            for mkt in markets.values():
                ...
                for rid in mkt.get("runnerIds",[]):
                    ...
                    if "over" not in rname.lower():
                        continue
                    handicap = float(runner.get("handicap") or 0)
                    player   = re.sub(r"\s*-.*","",rname).replace(" Over","").strip()
                    if not player or handicap <= 0:
                        continue
