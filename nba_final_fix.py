    ...
    for game in games:
        ...
        for loc,tid,opp_id,opp_name,side in [('Home',game['home_id'],a,a_name,'HOME'),('Away',game['away_id'],h,h_name,'AWAY')]:
            for player in rosters.get(tid,[]):
                ...
                for sk,sc in STAT_CONFIG.items():
                    ob_ud   = dk_lookup.get((_nn(pname),sk),{})
                    ob_odds = odds_lookup.get((_nn(pname),sk),{})
                    ob = ob_ud if ob_ud.get('line') else ob_odds
                    if not ob or ob.get('line') is None: continue
                    line = float(ob['line'])
                    opp_logs = [l for l in logs_by_player.get(pid,[]) if l['location']==loc and l['opp']==opp_id]
                    if not opp_logs:
                        props_nopick.append({'player':pname,'team':h_name if loc=='Home' else a_name,'stat':sk,'stat_label':sc['label'],'emoji':sc['emoji'],'side':side,'opp_name':opp_name,'line':line,'avg':None,'games':0,'hits':0,'hit_pct':0,'history':'—','gap':None,'pick':None,'fd_odds':ob.get('odds','')})
                        continue
                    # Cap at last 10 most recent games vs opponent
                    recent_logs = opp_logs[-10:]
                    vals = [float(l[sk]) for l in recent_logs]
                    avg = round(sum(vals)/len(vals),1)
                    gap = round(avg-line,1)
                    pick = 'OVER' if avg>line else ('UNDER' if avg<line else None)
                    hits  = sum(1 for v in vals if v > line)
                    hit_pct = round(hits/len(vals)*100) if vals else 0
                    team_name = h_name if loc=='Home' else a_name
                    entry = {'player':pname,'team':team_name,'stat':sk,'stat_label':sc['label'],'emoji':sc['emoji'],'side':side,'opp_name':opp_name,'line':line,'avg':avg,'games':len(vals),'hits':hits,'hit_pct':hit_pct,'history':','.join(str(int(v)) for v in vals),'gap':gap,'pick':pick,'fd_odds':ob.get('odds','')}
...
/* HEADER */
.hdr{background:#000;border-bottom:4px solid #FDB827;padding:22px 28px;text-align:left}
.hdr h1{font-size:1.5rem;font-weight:900;color:#fff;letter-spacing:2px;text-transform:uppercase;line-height:1}
...
function renderPropsSection(picks, nopick) {
  ...
  var ctr=document.getElementById('props-game-groups');
  if(!ctr) return;
  if(!all.length){
    ctr.innerHTML='<p style="text-align:center;padding:20px;color:#555">No prop lines available yet — check back closer to tip-off.</p>';
    return;
  }
  var groups={}, order=[];
  all.forEach(function(p){
    var key=p.side==='HOME'?(p.opp_name+' @ '+p.team):(p.team+' @ '+p.opp_name);
    if(!groups[key]){groups[key]=[];order.push(key);}
    groups[key].push(p);
  });
  var html='';
  order.forEach(function(matchup,mi){
    var gp=groups[matchup], gid='pg'+mi;
    html+='<div style="margin-bottom:14px">';
    // Collapsible game header
    html+='<div style="background:#111;border-left:4px solid #FDB827;padding:9px 14px;cursor:pointer;display:flex;justify-content:space-between;align-items:center;border-radius:0 6px 6px 0;margin-bottom:8px" onclick="var e=document.getElementById(''+gid+'');e.style.display=e.style.display==='none'?'block':'none';">';
    html+='<span style="font-weight:900;color:#fff;font-size:.85rem">🏀 '+matchup+'</span>';
    html+='<span style="background:rgba(253,184,39,.15);border:1px solid rgba(253,184,39,.3);color:#FDB827;padding:2px 10px;border-radius:20px;font-size:.75rem">'+gp.length+' props ▾</span>';
    html+='</div>';
    // Compact table — each row is one prop
    html+='<div id="'+gid+'" style="overflow-x:auto;border:1px solid #1a1a1a;border-radius:8px">';
    html+='<table style="width:100%;border-collapse:collapse;font-size:.8rem;background:#0d0d0d">';
    html+='<thead><tr style="border-bottom:1px solid #222">';
    html+='<th style="padding:7px 10px;text-align:left;color:#FDB827;font-size:.68rem;background:#111;white-space:nowrap">Player · Team</th>';
    html+='<th style="padding:7px 8px;text-align:left;color:#FDB827;font-size:.68rem;background:#111;white-space:nowrap">Stat</th>';
    html+='<th style="padding:7px 8px;text-align:center;color:#FDB827;font-size:.68rem;background:#111">H/A</th>';
    html+='<th style="padding:7px 8px;text-align:center;color:#FDB827;font-size:.68rem;background:#111">Line</th>';
    html+='<th style="padding:7px 8px;text-align:center;color:#FDB827;font-size:.68rem;background:#111">Avg</th>';
    html+='<th style="padding:7px 8px;text-align:center;color:#FDB827;font-size:.68rem;background:#111">Gap</th>';
    html+='<th style="padding:7px 8px;text-align:center;color:#FDB827;font-size:.68rem;background:#111;white-space:nowrap">L10VT H/A</th>';
    html+='<th style="padding:7px 10px;text-align:left;color:#FDB827;font-size:.68rem;background:#111;min-width:120px">History</th>';
    html+='<th style="padding:7px 8px;text-align:center;color:#FDB827;font-size:.68rem;background:#111">Pick</th>';
    html+='</tr></thead><tbody>';
    gp.forEach(function(p){
      var isOver=p.pick==='OVER', isUnder=p.pick==='UNDER';
      var pickClr=isOver?'#4ade80':isUnder?'#f87171':'#666';
      var rateClr=p.hit_pct>=75?'#4ade80':p.hit_pct>=60?'#FDB827':'#f87171';
      var gap=p.gap!=null?(p.gap>0?'+':'')+p.gap:'—';
      var rowBg=isOver?'rgba(74,222,128,.03)':isUnder?'rgba(248,113,113,.03)':'';
      html+='<tr style="border-bottom:1px solid #161616;background:'+rowBg+'">';
      // Player + team
      html+='<td style="padding:7px 10px;white-space:nowrap"><span style="font-weight:700;color:#f0e6c8">'+p.player+'</span><span style="display:block;font-size:.68rem;color:#FDB827">'+( p.team||'')+'</span></td>';
      // Stat
      html+='<td style="padding:7px 8px;color:#aaa;white-space:nowrap">'+p.emoji+' '+p.stat_label+'</td>';
      // H/A badge
      var sideBg=p.side==='HOME'?'rgba(253,184,39,.15)':'rgba(99,102,241,.2)';
      var sideClr=p.side==='HOME'?'#FDB827':'#a5b4fc';
      html+='<td style="padding:7px 8px;text-align:center"><span style="background:'+sideBg+';color:'+sideClr+';padding:1px 6px;border-radius:4px;font-size:.7rem;font-weight:700">'+p.side+'</span></td>';
      // Line / Avg / Gap
      html+='<td style="padding:7px 8px;text-align:center;font-family:monospace;font-weight:700;color:#fff">'+p.line+'</td>';
      html+='<td style="padding:7px 8px;text-align:center;font-family:monospace;font-weight:700;color:'+pickClr+'">'+(p.avg||'—')+'</td>';
      html+='<td style="padding:7px 8px;text-align:center;font-family:monospace;color:'+pickClr+';font-weight:700">'+gap+'</td>';
      // Hit rate
      html+='<td style="padding:7px 8px;text-align:center;white-space:nowrap"><span style="font-weight:700;color:'+rateClr+'">'+(p.hits||0)+'/'+p.games+'</span><span style="color:#555;font-size:.68rem"> ('+( p.hit_pct||0)+'%)</span></td>';
      // History — scrollable in its own cell
      html+='<td style="padding:7px 10px;max-width:160px"><div style="overflow-x:auto;font-family:monospace;font-size:.7rem;color:#555;white-space:nowrap">'+( p.history||'—')+'</div></td>';
      // Pick badge
      html+='<td style="padding:7px 8px;text-align:center"><span style="font-weight:900;font-size:1rem;color:'+pickClr+'">'+(isOver?'O':isUnder?'U':'—')+'</span></td>';
      html+='</tr>';
    });
