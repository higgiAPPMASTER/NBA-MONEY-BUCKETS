...
async def fetch_fd_nba_lines() -> Dict[str, Dict]:
    """Returns {player_name_lower: {PTS: line, REB: line, AST: line, FG3M: line}}"""
    ...
    try:
        ...
        async with CFSession(impersonate="chrome120") as s:
            ...
            if r.status_code != 200:
                print(f"[FanDuel NBA] status {r.status_code}")
                return {}
            data     = r.json()
            ...
            for mkt in markets.values():
                ...
                for rid in mkt.get("runnerIds",[]):
                    ...
                    # Skip unders
                    if "under" in rname_lower and "over" not in rname_lower:
                        continue
                    # Extract player name — NBA formats vary:
                    # "Jayson Tatum - Over 29.5" or "Jayson Tatum" or "Over 29.5 - Jayson Tatum"
                    player = rname
                    player = re.sub(r"[-\s]*(over|under)[\s\d\.]*", "", player, flags=re.IGNORECASE)
                    player = re.sub(r"^(over|under)[\s\d\.]+", "", player, flags=re.IGNORECASE)
        ...
        print(f"[FanDuel NBA] {len(lines)} players with lines")
        return lines
    except Exception as e:
