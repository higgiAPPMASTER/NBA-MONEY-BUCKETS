...
async def run_analysis(selected_date: str = None) -> Dict:
    ...
    log = []
    log.append(f"Fetching NBA schedule and sportsbook lines for {today_str}...")
    ...
    if not odds_props:
        log.append("No sportsbook lines found. Check ODDS_API_KEY in Render env vars.")
        return {'date': today_str, 'picks': [], 'all_picks': [], 'games': games,
                'log': log, 'total': 0, 'odds_loaded': False}

    log.append(f"{len(odds_props)} player prop lines loaded from The Odds API")
    ...
    for tid, players in rosters.items():
        for p in players:
            name_to_player[_nn(p['name'])] = {**p, 'team_id': tid}
    ...
    # Build team context: abbrev -> {opp, location, matchup, ...}
    team_ctx: Dict[str, Dict] = {}
    for game in games:
        matchup = f"{game['away_name']} @ {game['home_name']}"
        team_ctx[game['home']] = {'opp': game['away'], 'opp_name': game['away_name'],
                                   'location': 'Home', 'matchup': matchup,
                                   'team_id': game['home_id']}
        team_ctx[game['away']] = {'opp': game['home'], 'opp_name': game['home_name'],
                                   'location': 'Away', 'matchup': matchup,
                                   'team_id': game['away_id']}

    # Core: for each Odds API line, check historical hit rate vs today's opponent
    log.append(f"Checking {len(odds_props)} lines against historical data (75%+ threshold)...")
    picks = []
    ...
    for prop in odds_props:
        ...
        stat        = prop['stat']
        line        = prop['line']
        sc          = STAT_CONFIG.get(stat)
        if not sc:
            continue

        # Match player name to roster
        pn      = _nn(player_name)
        matched = name_to_player.get(pn)
        if not matched:
            for rn, rp in name_to_player.items():
                if _nm(player_name, rp['name']):
                    matched = rp
                    break
        if not matched:
            continue
        ...
        # Find team in today's games
        ctx = None
        team_abbr = None
        team_name = None
        for abbr, c in team_ctx.items():
            if c['team_id'] == team_id:
                ctx = c
                team_abbr = abbr
                team_name = matched['name']  # use roster name
                break
        if not ctx:
            continue
        ...
        # H/A games vs today's opponent
        opp_logs = [l for l in logs_by_player.get(pid, [])
                    if l['location'] == location and l['opp'] == opp]

        if len(opp_logs) < MIN_GAMES:
            continue

        # Hit rate vs sportsbook line
        hits  = sum(1 for l in opp_logs if float(l.get(stat, 0)) > line)
        total = len(opp_logs)
        rate  = hits / total
        ...
        # Last 10 vs this team
        last10   = opp_logs[:10]
        l10_hits = sum(1 for l in last10 if float(l.get(stat, 0)) > line)
        l10_games = len(last10)
    ...
    picks.sort(key=lambda x: (x['hit_rate'], x['l10_hits']), reverse=True)
    top_picks = picks[:TOP_N]
    log.append(f"{len(picks)} players qualify at 75%+ hit rate vs sportsbook line -> top {TOP_N} shown")

    result = {'date': today_str, 'picks': top_picks, 'all_picks': picks,
              'games': games, 'log': log, 'total': len(picks), 'odds_loaded': True}
...
function renderTop10Cards(picks){
  ...
  picks.forEach((p,i)=>{
    ...
    <div class="pick-card">
      ...
      <div class="fd-line-badge">Line: <strong>${p.fd_line}</strong> ${p.odds ? "(" + p.odds + ")" : ""} · ${p.hits}/${p.games} ${p.location.toLowerCase()} games vs ${p.opp} went over</div>
      ${p.l10_games > 0 ? `<div class="l10vthr-desc">${p.player.split(" ").pop()} hit ${p.fd_line}+ ${p.stat_label} ${p.l10_hits} of ${p.l10_games} last 10 games vs ${p.opp}</div>` : ""}
